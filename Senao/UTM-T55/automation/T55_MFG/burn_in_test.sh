...
sshpass -p senao ssh root@192.168.0.2 "/root/automation/T55_MFG/burn_in_test_golden.sh" | tee -a /root/automation/log.txt | tee /tmp/log_tmp_golden.txt &
sleep 1
/root/automation/T55_MFG/stress.sh --no_stress --no_disk | tee -a /root/automation/log.txt | tee /tmp/log_tmp.txt
sleep 2
stress_iperf=$( cat /tmp/log_tmp.txt | grep "total" | awk '{ print $5 }' )
sed -i '$d' /root/automation/test_results.txt
...
#echo "[DEBUG] ERROR =  $ERROR" | tee -a /root/automation/log.txt
if [ "$ERROR" == "0" ]; then
