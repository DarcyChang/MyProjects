...
function eth_stat() {
	...
	if [ ! -f $rate_tmp_file ] || [ ! -s $rate_tmp_file ] ; then
		echo ""
		echo "[ERROR] ERROR happen."
		ls -al $rate_tmp_file
		cat $rate_tmp_file 
		sar -n DEV $eth_stat_interval 1 | grep Average
		echo ""
	fi
    eth_idx=0
    eth_stat_msg=""
...
function show_stat() {
	...
	printf "[$mode] %02d:%02d:%02d ${cpu_stat_msg} ${mem_stat_msg} ${eth_stat_msg}\n" $hour $min $sec
}
...
if [ "$no_iperf" == "0" ]; then
    ...
    do
        ...
        echo "[$mode] ${eth_name[$eth_idx]} check average rx/tx rate: [$result] [$average_rx_rate/$average_tx_rate Mbits/sec]"
        if [ "$is_vlan_eth" == "0" ]; then
            ...
            echo "[$mode] ${eth_name[$eth_idx]} check error counter:      [$result] [RX:$rx_errors TX:$tx_errors]"
            if [ "$rx_errors" != "0" ] || [ "$tx_errors" != "0" ]; then
        ...
        echo "[$mode] ${eth_name[$eth_idx]} check link status:        [$result] [detected ${eth_link_down_count[$eth_idx]} time(s) of port link down]"
        eth_idx=`expr $eth_idx + 1`
    done
    if [ "$total_rx_rate" -lt "$total_rate_min" ] || [ "$total_tx_rate" -lt "$total_rate_min" ]; then
   	...
   	echo "[$mode] check total rx/tx rate: [$result] [$total_rx_rate/$total_tx_rate Mbits/sec]"
