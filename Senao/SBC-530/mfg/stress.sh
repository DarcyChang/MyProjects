...
rate_criteria=10
disk_count=0
usb_time=300 # usb stress 300 seconds
...
[ -z "$sec" ] && sec=0
if [[ $time -gt $usb_time ]] ; then
	time=$(echo "( $day * 3600 * 24 ) + ( $hour * 3600 ) + ( $min * 60 ) + $sec - $usb_time" | bc)
else
   ...
   if [ "$extra_disk_count" -lt "$disk_count" ]; then
    ...
    for disk in $disk_list; do
        ...
        if [ "$disk" != "$sys_disk" ]; then
			...
			if [ -n "$(udevadm info /dev/$disk | grep DEVPATH | grep '/usb')" ] ; then
				stress_usb_arg="${stress_usb_arg}-f /tmp/${disk}/${tmp_file}${file_id} "
			else
...
rm /tmp/time.txt 2> /dev/null
echo "stress START $(date '+%Y-%m-%d %H:%M:%S')" >> /tmp/time.txt
time=$usb_time
start_time=$(cat /proc/uptime | cut -d. -f1)
end_time=$(expr $start_time + $time)
echo "[DEBUG] stressapptest $stress_usb_arg -s $time -v 4"
stressapptest $stress_usb_arg -s $time -v 4 > /tmp/stress_res &

echo "USB stress test running"
sleep 2
stress_running=1
...
if [ -z "$no_disk" ]; then
    file_id=1
    for disk in $disk_list; do
        rm /tmp/${disk}/${tmp_file}${file_id} 2> /dev/null
        file_id=$(( file_id + 1 ))
    done
fi

echo "-------------------------------------------------------"
usb_fail=$(grep -c "FAIL" /tmp/stress_res)
if [[ $usb_fail == "1" ]] ; then
	echo "[ERROR] USB stress fail."
	cat /tmp/stress_res
	echo "stress end usb $(date '+%Y-%m-%d %H:%M:%S')" >> /tmp/time.txt
	sleep 3
	exit 1
fi
echo "stress end usb $(date '+%Y-%m-%d %H:%M:%S')" >> /tmp/time.txt
...
start_time=$(cat /proc/uptime | cut -d. -f1)
end_time=$(expr $start_time + $time)
echo "[DEBUG] stressapptest $stress_cpu_arg $stress_mem_arg $stress_disk_arg $stress_filesize_arg -s $time -v 4"
stressapptest $stress_cpu_arg $stress_mem_arg $stress_disk_arg $stress_filesize_arg -s $time -v 4 > /tmp/stress_res &

echo "CPU/Memory/Disk/Network stress test running"
sleep 2
stress_running=1
while [ "$stress_running" != "0" ]
do
...
echo "stress END $(date '+%Y-%m-%d %H:%M:%S')" >> /tmp/time.txt
