...
*** Test Cases ***
tc_AndroidAPP_054_MainScreen_Change_the_Zap_network_SSID
    [Documentation]  tc_AndroidAPP_054_MainScreen_Change_the_Zap_network_SSID
    ...
    ...    5. Change the Zap SSID
    ...    6. Save settings and check the status
    [Tags]   @TCID=WRTM-326ACN-231    @DUT=WRTM-326ACN     @AUTHOR=Gavin_Chang
    [Timeout]
    ...
    Change the Zap SSID
    Save settings and check the status
...
Press the ZAP button
    ...
    Set Test Variable    ${wifi_ip}    ${wifi_ip}

Change the Zap SSID
    [Documentation]
    [Tags]   @AUTHOR=Gavin_Chang
    touch ZAP setting
    Change ZAP SSID name    ${g_app_rename_guest_ssid}
...
Save settings and check the status
    ...
    touch confirm
    wait main screen
    Wait Until Keyword Succeeds    5x    3s    Is Linux Ping Fail    dut1    ${wifi_ip}
