...
*** Test Cases ***
tc_2.4G_Wireless_Security_Type_Is_WPA_PSK_connection_with_incorrect_password
    [Documentation]  tc_2.4G_Wireless_Security_Type_Is_WPA_PSK_connection_with_incorrect_password
    ...    1. Turn on wireless radio and setting SSID on the DUT.
    ...    2. Setting security type is WPA-PSK on the DUT.
    ...
    [Tags]   @TCID=WRTM-326ACN-456    @DUT=WRTM-326ACN     @AUTHOR=Hans_Sun
    [Timeout]

    Turn on wireless radio and setting SSID on the DUT
    Setting security type is WPA-PSK on the DUT
...
Setting security type is WPA-PSK on the DUT
    [Tags]   @AUTHOR=Hans_Sun
    Config Wireless Home Network    web    Security=WPA-PSK
