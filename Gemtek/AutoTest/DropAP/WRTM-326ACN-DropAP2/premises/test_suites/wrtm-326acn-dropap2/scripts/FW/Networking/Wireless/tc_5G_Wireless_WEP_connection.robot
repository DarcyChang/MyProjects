...
*** Test Cases ***
tc_5G_Wireless_WEP_connection
    [Documentation]  tc_5G_Wireless_WEP_connection
    ...
    [Tags]   @TCID=WRTM-326ACN-427    @DUT=WRTM-326ACN     @AUTHOR=Hans_Sun
    [Timeout]
...
5GWIFI client connect to the DUT, verify DUT leased IP to WIFI client
    [Tags]   @AUTHOR=Hans_Sun
    Wait Until Keyword Succeeds    3x    1s    Login Linux Wifi Client To Connect To DUT Using WEP Encryption    wifi_client    ${g_dut_home_ssid}-5G    ${g_dut_repeater_ssid_pw}    ${DEVICES.wifi_client.int}    ${g_dut_gw}
