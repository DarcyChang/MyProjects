...
*** Test Cases ***
tc_SPI_Firewall_with_block_SYN_Flood_attack
    [Documentation]  tc_SPI_Firewall_with_block_SYN_Flood_attack
    ...    1. Setup WAN host and connected DUT WAN side.
    ...    2. WAN host simulate SYN Flood attack to DUT WAN IP and DUT can block this attack
    [Tags]   @TCID=WRTM-326ACN-323    @DUT=WRTM-326ACN     @AUTHOR=Hans_Sun
    [Timeout]

    Setup WAN host and connected DUT WAN side
    WAN host simulate SYN Flood attack to DUT WAN IP and DUT can block this attack
...
Kill Ping Command
    [Documentation]
    [Tags]   @AUTHOR=Hans_Sun
    cli    wanhost    echo '${DEVICES.wanhost.password}' | sudo -S killall -9 hping3
