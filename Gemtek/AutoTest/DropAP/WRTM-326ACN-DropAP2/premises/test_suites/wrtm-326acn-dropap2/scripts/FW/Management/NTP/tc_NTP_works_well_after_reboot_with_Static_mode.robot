...
*** Variables ***
*** Test Cases ***
tc_NTP_works_well_after_reboot_with_Static_mode
    [Documentation]  tc_NTP_works_well_after_reboot_with_Static_mode
    ...    1. Change the static WAN connection types.
    ...    2. DUT reboot on static WAN connection types, verify NTP works well after DUT boot up.

    [Tags]   @TCID=WRTM-326ACN-333    @DUT=WRTM-326ACN     @AUTHOR=Jujung_Chang
    [Timeout]

    Change the DHCP WAN connection types
    DUT reboot on static WAN connection types, verify NTP works well after DUT boot up
