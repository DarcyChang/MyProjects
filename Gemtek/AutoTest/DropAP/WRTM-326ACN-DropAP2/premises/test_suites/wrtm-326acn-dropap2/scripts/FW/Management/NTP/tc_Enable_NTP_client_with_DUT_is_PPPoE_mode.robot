...
*** Variables ***
*** Test Cases ***
tc_Enable_NTP_client_with_DUT_is_PPPoE_mode
    [Documentation]  tc_Enable_NTP_client_with_DUT_is_PPPoE_mode
    ...    1. Change the static WAN connection types.
    ...    2. Checking the time is correct.Verify NTP can work well on PPPoE mode.

    [Tags]   @TCID=WRTM-326ACN-327    @DUT=WRTM-326ACN     @AUTHOR=Jujung_Chang
    [Timeout]

    Change the DHCP WAN connection types
    Checking the time is correct.Verify NTP can work well on PPPoE mode
