...
from robotide.publish.messages import RideMessage, RideTestSelectedForRunningChanged
...
from plugins.CafeTextEditor.breakpoint_manager import BreakPointRecoder

try:
    from robotide.editor import robotframeworklexer
except Exception as e:
    robotframeworklexer = None


class CafeTextEditorPlugin(Plugin, TreeAwarePluginMixin):
    title = 'Cafe Text Edit'

    def __init__(self, application):
        Plugin.__init__(self, application, name=self.title)
        ...
        def focused(func):
            def f(event):
                if self.is_focused() and self._editor.is_focused():
                    func(event)
            return f
        self.register_shortcut('Alt-S', focused(lambda e: self._editor.save()))
    ...
    def OnTestSelectedForRunningChanged(self, message):
        self._editor._names_to_run = message.tests
...
class SourceEditor(wx.Panel):
    ...
    def __init__(self, parent, title, data_validator):
        ...
        self.break_point_manager = BreakPointRecoder()
        self._plugins_settings = parent._app.settings['Plugins']
        self.robotfile = tempfile.NamedTemporaryFile(mode='w+t', suffix='.robot')
    ...
    def _create_editor_toolbar(self):
        ...
        # Add a debugger button
        self.debugger_button = wx.Button(self, label='Run Debugger', size=(-1, 25))
        default_components.add_with_padding(self.debugger_button)
        self.debugger_button.Bind(wx.EVT_BUTTON, self.OnRunDebugger)
        self.debugger_button.Disable()
    ...
    def OnRunDebugger(self, event):
        self.debug_file_generator()
        self.start_debugger_deamon()
        self.start_robot()
    ...
    def _create_editor_text_control(self, text=None):
        ...
        self._editor.Bind(stc.EVT_STC_MARGINCLICK, self.OnMarginClick)

    def OnMarginClick(self, event):
        if event.GetMargin() == 1:
            lineClicked = self._editor.LineFromPosition(event.GetPosition())
            if self._editor.MarkerGet(lineClicked):
                self._editor.MarkerDelete(lineClicked, self._editor.break_point_maker)
                self.break_point_manager.del_breakpoint(lineClicked, self.datafile_controller.filename)
                if not self.break_point_manager.bps:
                    self.debugger_button.Disable()

            else:
                self._editor.MarkerAdd(lineClicked, self._editor.break_point_maker)
                self.break_point_manager.add_breakpoint(lineClicked, self.datafile_controller.filename)
                if not self.debugger_button.IsEnabled():
                    self.debugger_button.Enable()
    ...
    def debug_file_generator(self):
        fh = open(self.datafile_controller.filename)
        flist = fh.readlines()
        count = 0
        for bp_obj in self.break_point_manager.get_breakpoints():
            if bp_obj.location == self.datafile_controller.filename:
                insert_line = bp_obj.line + count
                indent = ' ' * (len(flist[bp_obj.line]) - len(flist[bp_obj.line].lstrip()))
                flist.insert(insert_line,
                             indent + 'breakpoint on {}:{}\n'.format(bp_obj.location, bp_obj.line + 1))
                count += 1

        open(self.robotfile.name, 'w').writelines(flist)

    def start_debugger_deamon(self):
        os.system("ps -ef | grep rwb.debugger | grep -v grep | awk '{print $2}' | xargs kill > /dev/null 2>&1")
        sleep(1)
        os.system('python -m rwb.debugger&')
        sleep(1)
    ...
    def start_robot(self):
        ...
        sleep(1)
        self.robotfile.close()

    def _get_plugin_settings(self, names):
        if isinstance(names, list):
            for name in names:
                if self._plugins_settings.has_setting(name) and \
                        self._plugins_settings[name]['_enabled']:
                    return self._plugins_settings[name]
        return None

    def _create_command(self):
        plugin_settings = self._get_plugin_settings(['Cafe Test Runner', 'Test Runner'])
        if not plugin_settings:
             print '[Warnings] This test suite was not tested by Cafe Test Runner of Test Runner Plugin!'
        else:
            profile_name = plugin_settings['profile'].strip().replace(' ', '_')
            if profile_name not in ['pybot', 'jybot']:
                runner_script = plugin_settings.get('{}_runner_script'.format(profile_name), '')
            else:
                runner_script = profile_name

        orig_arg = plugin_settings.get('{}_arguments'.format(profile_name), '')
        arguments = self._sub_argu_with_debug_info(orig_arg)
        include_tags = plugin_settings.get('{}_include_tags'.format(profile_name), '')
        exclude_tags = plugin_settings.get('{}_exclude_tags'.format(profile_name), '')

        seleted_test = ''
        for _, test in self._names_to_run:
            seleted_test = '--test "{}"'.format(test.split('.')[-1])

        command = '{} {} {} {} {} {}'.format(runner_script,
                                          arguments,
                                          include_tags,
                                          exclude_tags,
                                          seleted_test,
    ...
    def _sub_argu_with_debug_info(self, argument):
        if '--loglevel' in argument:
            loglevel_reg = re.compile(r'--loglevel\s+\S+')
            argument = re.sub(loglevel_reg, '--loglevel DEBUG', argument)
        else:
            argument += ' --loglevel DEBUG '

        if '--listener' in argument:
            listener_reg = re.compile('--listener\s+\S+')
            argument = re.sub(listener_reg, '--listener rwb.socket_listener:8910', argument)
        else:
            argument += ' --listener rwb.socket_listener:8910 '
...
class RobotDataEditor(stc.StyledTextCtrl):
    ...
    def __init__(self, parent):
        self.break_point_maker = 1
        stc.StyledTextCtrl.__init__(self, parent)
        self.SetMarginType(0, stc.STC_MARGIN_NUMBER)
        self.SetMarginWidth(0, self.TextWidth(stc.STC_STYLE_LINENUMBER,'1234'))

        # add a margin type for break point
        self.SetMarginType(1, stc.STC_MARGIN_SYMBOL)
        self.MarkerDefine(self.break_point_maker, stc.STC_MARK_CIRCLE, None, "RED")
