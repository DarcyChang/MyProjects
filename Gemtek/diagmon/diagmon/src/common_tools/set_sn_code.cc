...
DIAG_CODE set_sn_code_uid_handle(IO_DATA *pio_data, DIAG_RESULT *prst)
{
	...
	if((pio_data->argc == 4) && (strncmp(pio_data->argv[UID_IDX+1], "help", 4) == 0)) {
		printf("diagmon -c <uid string> <sn>\n");
		printf("EX: diagmon -c cmn-cmn07 SN12345678\n");
	}else if(pio_data->argc == (3 + params_num)) {
		strncpy(pio_data->u.cmt.sn, pio_data->argv[UID_IDX+1], 31);
		ret = run_set_sn_code(pio_data, prst);
	}else {
