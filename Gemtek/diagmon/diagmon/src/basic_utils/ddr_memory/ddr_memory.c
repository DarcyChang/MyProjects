...
#include "ddr_memory_com.cc"
#include "show_memory_info.cc"
#include "run_memory_0_test.cc"
#include "run_memory_1_test.cc"
#include "run_memory_random_test.cc"
#include "run_memory_sampling_test.cc"
...
DIAG_CODE ddr_memory_utils(IO_DATA *pio_data, DIAG_RESULT *prst)
{
	...
	DIAGS_NODE *menu = diag_basic_ddr_memory_menu;

	diag_menu_index_init(menu, SIZE_DDR_MEMORY_MENU);

	while (1) {
		flag_valid_menu = UNSET;
		show_menu(menu, SIZE_DDR_MEMORY_MENU, pio_data);
		...
		if(option == DONE) {
			for(i = 1; i < SIZE_DDR_MEMORY_MENU; i++) {
