	...
	*/
	...
	do{
		...
		memset(cmd, 0, sizeof(cmd));
                sprintf(cmd, "memtester -p %x %dM 1 13", offset, size);
		local_ret = system(cmd);

		/* report the result */
		/* The secondary parameter of diag_report() must be PASS or FAIL */
		if(local_ret == -1) {
			ret = diag_report(prst, FAIL, "memory_1_test fail report");
		}else if(local_ret == 0){
			ret = diag_report(prst, PASS, "memory_1_test pass report");
		}else{
			// get unexpected ret
		}
...
DIAG_CODE memory_1_test_uid_handle(IO_DATA *pio_data, DIAG_RESULT *prst)
{
	...
	if((pio_data->argc == 4) && (strncmp(pio_data->argv[UID_IDX+1], "help", 4) == 0)) {
		...
		printf("EX: diagmon -c bsc-mem-mem03 0x1200000 1\n");
		...
		ret = run_memory_1_test(pio_data, prst);
	}else {
