...
#include "usb.h"
#include "usb_com.cc"
#include "show_usb_info.cc"
#include "usb_traffic_test.cc"
#include "usb_insertion_test.cc"
#include "usb_pattern_test.cc"
...
DIAG_CODE usb_utils(IO_DATA *pio_data, DIAG_RESULT *prst)
{
	...
	DIAGS_NODE *menu = diag_basic_usb_menu;

	diag_menu_index_init(menu, SIZE_USB_MENU);

	while (1) {
		flag_valid_menu = UNSET;
		show_menu(menu, SIZE_USB_MENU, pio_data);
