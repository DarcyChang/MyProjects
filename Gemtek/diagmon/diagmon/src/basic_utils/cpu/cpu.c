...
#include "cpu.h"
#include "cpu_com.cc"
#include "cpu_full_loading.cc"
#include "show_cpu_usage.cc"
#include "set_gpio.cc"
#include "get_gpio.cc"
...
DIAG_CODE cpu_utils(IO_DATA *pio_data, DIAG_RESULT *prst)
{
	...
	DIAGS_NODE *menu = diag_basic_cpu_menu;

	diag_menu_index_init(menu, SIZE_CPU_MENU);

	while (1) {
		flag_valid_menu = UNSET;
		show_menu(menu, SIZE_CPU_MENU, pio_data);
