...
DIAG_CODE turn_on_all_leds_uid_handle(IO_DATA *pio_data, DIAG_RESULT *prst)
{
	...
	if((pio_data->argc == 4) && (strncmp(pio_data->argv[UID_IDX+1], "help", 4) == 0)) {
		printf("diagmon -c <uid string> <color_index>\n");
		printf("EX: diagmon -c bsc-led-led01 1\n");
	}else if(pio_data->argc == (3 + params_num)) {
		sscanf (pio_data->argv[UID_IDX+1] , "%d" , &color_idx);
		pio_data->u.led.color_idx = color_idx;
		ret = run_turn_on_all_leds(pio_data, prst);
	}else {
