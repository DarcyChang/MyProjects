...
#include "ethernet_phy_com.cc"
#include "get_phy_mac.cc"
#include "set_phy_mac.cc"
...
DIAG_CODE ethernet_phy_utils(IO_DATA *pio_data, DIAG_RESULT *prst)
{
	...
	DIAGS_NODE *menu = diag_basic_ethernet_phy_menu;

	diag_menu_index_init(menu, SIZE_ETHERNET_PHY_MENU);

	while (1) {
		flag_valid_menu = UNSET;
		show_menu(menu, SIZE_ETHERNET_PHY_MENU, pio_data);
		...
		if(option == DONE) {
			for(i = 1; i < SIZE_ETHERNET_PHY_MENU; i++) {
