...
DIAG_CODE run_reset_btn_test(IO_DATA *pio_data, DIAG_RESULT *prst)
{
	...
	*/
	...
	do{
		...
		if(local_ret == ERROR) {
			ret = diag_report(prst, FAIL, "internal reset_btn_test fail report");
		}else if(local_ret == DONE){
			ret = diag_report(prst, PASS, "internal reset_btn_test pass report");
		}else{
			// get unexpected ret
		}
...
DIAG_CODE reset_btn_test_uid_handle(IO_DATA *pio_data, DIAG_RESULT *prst)
{
	...
	if((pio_data->argc == 4) && (strncmp(pio_data->argv[UID_IDX+1], "help", 4) == 0)) {
		...
		printf("EX: diagmon -c bsc-btn-btn01\n");
