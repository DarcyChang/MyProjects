...
DIAG_CODE run_btn_test(IO_DATA *pio_data, DIAG_RESULT *prst)
{
	...
	*/
	...
	do{
		...
		if(local_ret == -1) {
			ret = diag_report(prst, FAIL, "btn_test fail report");
		}else if(local_ret == 0){
			ret = diag_report(prst, PASS, "btn_test pass report");
		}else{
			// get unexpected ret
		}
...
DIAG_CODE btn_test_uid_handle(IO_DATA *pio_data, DIAG_RESULT *prst)
{
	...
	if((pio_data->argc == 4) && (strncmp(pio_data->argv[UID_IDX+1], "help", 4) == 0)) {
		...
		printf("EX: diagmon -c bsc-btn-btn03\n");
	}else if(pio_data->argc == (3 + params_num)) {
		ret = run_btn_test(pio_data, prst);
	}else {
