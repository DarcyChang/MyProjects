#include <diag_cmm.h>
#include "hw_button.h"
#include "hw_button_com.cc"
#include "internal_reset_btn_test.cc"
#include "external_reset_btn_test.cc"
#include "btn_test.cc"
...
DIAG_CODE hw_button_utils(IO_DATA *pio_data, DIAG_RESULT *prst)
{
	...
	DIAGS_NODE *menu = diag_basic_hw_button_menu;

	diag_menu_index_init(menu, SIZE_HW_BUTTON_MENU);

	while (1) {
		flag_valid_menu = UNSET;
		show_menu(menu, SIZE_HW_BUTTON_MENU, pio_data);
		...
		if(option == DONE) {
			for(i = 1; i < SIZE_HW_BUTTON_MENU; i++) {
