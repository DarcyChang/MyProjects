#include <diag_cmm.h>
#include "sample.h"
#include "sample_com.cc"
#include "sample_tool.cc"
...
DIAG_CODE sample_utils(IO_DATA *pio_data, DIAG_RESULT *prst)
{
	...
	DIAGS_NODE *menu = diag_basic_sample_menu;

	diag_menu_index_init(menu, SIZE_SAMPLE_MENU);

	while (1) {
		flag_valid_menu = UNSET;
		show_menu(menu, SIZE_SAMPLE_MENU, pio_data);
