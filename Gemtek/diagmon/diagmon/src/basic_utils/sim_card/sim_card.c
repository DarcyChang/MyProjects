#include <diag_cmm.h>
#include "sim_card.h"
#include "sim_card_com.cc"
#include "show_sim_card_info.cc"
#include "show_imsi_info.cc"
...
DIAG_CODE sim_card_utils(IO_DATA *pio_data, DIAG_RESULT *prst)
{
	...
	DIAGS_NODE *menu = diag_basic_sim_card_menu;

	diag_menu_index_init(menu, SIZE_SIM_CARD_MENU);

	while (1) {
		flag_valid_menu = UNSET;
		show_menu(menu, SIZE_SIM_CARD_MENU, pio_data);
		...
		if(option == DONE) {
			for(i = 1; i < SIZE_SIM_CARD_MENU; i++) {
