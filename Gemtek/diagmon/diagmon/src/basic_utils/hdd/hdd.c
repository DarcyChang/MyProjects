...
#include "hdd.h"
#include "hdd_com.cc"
#include "show_hdd_info.cc"
#include "hdd_traffic_test.cc"
...
DIAG_CODE hdd_utils(IO_DATA *pio_data, DIAG_RESULT *prst)
{
	...
	DIAGS_NODE *menu = diag_basic_hdd_menu;

	diag_menu_index_init(menu, SIZE_HDD_MENU);

	while (1) {
		flag_valid_menu = UNSET;
		show_menu(menu, SIZE_HDD_MENU, pio_data);
