...
DIAG_CODE continuous_tx_transmission_tool_uid_handle(IO_DATA *pio_data, DIAG_RESULT *prst)
{
	...
	if((pio_data->argc == 4) && (strncmp(pio_data->argv[UID_IDX+1], "help", 4) == 0)) {
		...
		printf("EX: diagmon -c bsc-wln-wln03 10\n");
	}else if(pio_data->argc == (3 + params_num)) {
		sscanf (pio_data->argv[UID_IDX+1] , "%d" , &second);
		pio_data->u.wl.time_sec = second;

		ret = run_continuous_tx_transmission_tool(pio_data, prst);
	}else {
