#include <diag_cmm.h>
#include "watchdog.h"
#include "watchdog_com.cc"
#include "show_watchdog_info.cc"
#include "watchdog_test.cc"
...
DIAG_CODE watchdog_utils(IO_DATA *pio_data, DIAG_RESULT *prst)
{
	...
	DIAGS_NODE *menu = diag_basic_watchdog_menu;

	diag_menu_index_init(menu, SIZE_WATCHDOG_MENU);

	while (1) {
		flag_valid_menu = UNSET;
		show_menu(menu, SIZE_WATCHDOG_MENU, pio_data);
		...
		if(option == DONE) {
			for(i = 1; i < SIZE_WATCHDOG_MENU; i++) {
