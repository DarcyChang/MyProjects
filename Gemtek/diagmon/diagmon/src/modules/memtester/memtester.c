...
void usage(char *me) {
    fprintf(stderr, "\nUsage: %s [-p physaddrbase] <mem>[B|K|M|G] [loops] [test function num]\n"
    "     test function num  0 : Random value\n"
    "                        10: Checkboard\n"
    "                        13: Walking 0\n"
    "                        14: Walking 1\n"
    "                        15: 8-bit write\n", me);
...
int main(int argc, char **argv) {
    ul loops, loop, i, num;
    size_t pagesize, wantraw, wantmb, wantbytes, wantbytes_orig, bufsize=0,
         halflen, count;
    char *memsuffix, *addrsuffix, *loopsuffix, *numsuffix;
    ...
    unsigned long file_crc = 0, device_crc = 0;
...
//    printf("stop address : 0x%08lx\n", bufb);
	...
	i = strtoul(argv[optind], &numsuffix, 0);
        if (!tests[i].name) break;
        printf("  %-13s ", tests[i].name);
        if (!tests[i].fp(bufa, bufb, count)) {
//            printf("Passed\n\n");
//	    printf("%-13s Test is PASSED\n", tests[i].name);
	    device_crc = crc32(buf, wantbytes);
//	    printf("file_crc:[%lu] device_crc:[%lu]\n", file_crc, device_crc);
        } else {
