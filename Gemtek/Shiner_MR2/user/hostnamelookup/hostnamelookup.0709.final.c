...
#define DEFAULT_TIMEOUT 86400 // 24 hours = 86400 second
...
void hostnamelookup_addv2(char* ip, char* hostname, int len)
{
	...
	int IP_timeout = 1800; // Set half-hour, ensure to delete after one hour.	
	...
	if(rf = fopen("/tmp/hostname_timeout_set","r")){
    	fscanf(rf,"%d",&timeout);
		fclose(rf);
	}
    ...
    for( i=4 ; i<( row + 1 ) * column ; i+=4 )
	{
		if(0 == strcmp(result[i],ip)){
			...
			printf("From db search HostName = %s\n", hostname);
	        snprintf(sqlcmd,256,"UPDATE result set timeout=%d, date='%s' where ipv4='%s' and hostname='%s';",timeout,ctime(&insert_date),result[i],result[i+1]);
    ...
    snprintf(sqlcmd,128, "SELECT * FROM IPlist ;");
		
	/* Get all hostname from IPlist table */
    sqlite3_get_table(IPlist_db, sqlcmd, &result, &row, &column, &errMsg);
	
	if(row == 0){
    	snprintf(sqlcmd,256,"INSERT INTO IPlist (ipv4, timeout) VALUES('%s',%d);", ip, IP_timeout);
		...
		sqlite3_finalize(stmt);
		if(row == 10){
    		snprintf(sqlcmd,256, "DELETE FROM IPlist where ipv4='%s' ;", result[2]);
			sqlite3_prepare_v2(IPlist_db,sqlcmd,256, &stmt, NULL);
			sqlite3_step(stmt);
    		sqlite3_reset(stmt);
			sqlite3_finalize(stmt);
...
void hostnamelookup_result(char* ip, char* hostname)
{
    ...
    snprintf(sqlcmd,128, "SELECT * FROM hostnametmp where ipv4='%s' ;", ip);
	...
	if(row == 0){
		printf("Database resultTMP is empty or can not find hostname by %s\n",ip);
   		snprintf(sqlcmd,128, "DELETE FROM IPlist where ipv4='%s' ;", ip);
		sqlite3_prepare_v2(IPlist_db,sqlcmd,256, &stmt, NULL);
		sqlite3_step(stmt);
   		sqlite3_reset(stmt);
		sqlite3_finalize(stmt);	
    ...
    snprintf(sqlcmd,128, "SELECT * FROM hostnametmp where ipv4='%s' and protocol='%s' ;", ip,"netbios");
	if(sqlite3_get_table(tmp_db, sqlcmd, &tmp, &row, &column, &errMsg) != SQLITE_OK)
		fprintf(stderr,"search table resultTMP error!\n");	
	if(row > 0){
    	snprintf(sqlcmd,256,"INSERT INTO result (ipv4, hostname, timeout, date) VALUES('%s', '%s', %d, '%s');", ip, tmp[4], timeout, ctime(&insert_date));
	    sqlite3_prepare_v2(hostname_db,sqlcmd,256, &stmt, NULL);
        sqlite3_step(stmt);
    	sqlite3_reset(stmt);
		sqlite3_finalize(stmt);
    	snprintf(sqlcmd,128, "DELETE FROM hostnametmp where ipv4='%s' ;", ip);
		sqlite3_prepare_v2(tmp_db,sqlcmd,128, &stmt, NULL);
    	sqlite3_step(stmt);
    	sqlite3_reset(stmt);
    	sqlite3_finalize(stmt);
		/* Release table */
    ...
    snprintf(sqlcmd,128, "SELECT * FROM hostnametmp where ipv4='%s' and protocol='%s' ;", ip,"bonjour");
	if(sqlite3_get_table(tmp_db, sqlcmd, &tmp, &row, &column, &errMsg) != SQLITE_OK)
		fprintf(stderr,"search table resultTMP error!\n");	
	if(row > 0){
    	snprintf(sqlcmd,256,"INSERT INTO result (ipv4, hostname, timeout, date) VALUES('%s', '%s', %d, '%s');", ip, tmp[4], timeout, ctime(&insert_date));
    ...
    snprintf(sqlcmd,128, "SELECT * FROM hostnametmp where ipv4='%s' and protocol='%s' ;", ip,"dnsquery");
	if(sqlite3_get_table(tmp_db, sqlcmd, &tmp, &row, &column, &errMsg) != SQLITE_OK)
		fprintf(stderr,"search table resultTMP error!\n");	
	if(row > 0){
    	snprintf(sqlcmd,256,"INSERT INTO result (ipv4, hostname, timeout, date) VALUES('%s', '%s', %d, '%s');", ip, tmp[4], timeout, ctime(&insert_date));
...
void addv2_dns(char* ip, char* hostname)
{
	...
	if(rf = fopen(HOSTNAME_TIMEOUT_SET,"r")){
    	fscanf(rf,"%d",&timeout);
		fclose(rf);
	}
...
void hostnamelookup_add(char* ip, char* hostname, int len)
{
    ...
    if(found == 1){
       	printf("\nHostName is %s\n", hostname);
    	snprintf(sqlcmd,256,"INSERT INTO result (ipv4, hostname, timeout, date) VALUES('%s', '%s', %d, '%s');", ip, hostname, timeout, ctime(&insert_date));
	    sqlite3_prepare_v2(hostname_db,sqlcmd,256, &stmt, NULL);
        sqlite3_step(stmt);
    	sqlite3_reset(stmt);
		sqlite3_finalize(stmt);
...
void hostnamelookup_del(char* ip)
{
    ...
    snprintf(sqlcmd,128, "DELETE FROM result where ipv4='%s' ;", ip);                                          
    sqlite3_prepare_v2(hostname_db,sqlcmd,128, &stmt, NULL);
	...
	snprintf(sqlcmd,128, "DELETE FROM IPlist where ipv4='%s' ;", ip);
	sqlite3_prepare_v2(IPlist_db,sqlcmd,128, &stmt, NULL);
	sqlite3_step(stmt);
    sqlite3_reset(stmt);
	sqlite3_finalize(stmt);
...
{
    sqlite3 *hostname_db;
    char sqlcmd[128] = "SELECT * FROM result;"; 
    char *errMsg = NULL;	
	int row=0, column=0;
	char **result;
	int i = 0,rc;
...
void hostnamelookup_del_all() 
{
    sqlite3 *hostname_db, *IPlist_db;
	sqlite3_stmt *stmt;
    char sqlcmd[128] = "DELETE FROM result;";
	int rc;
    ...
    snprintf(sqlcmd,128, "DELETE FROM IPlist ;");
	sqlite3_prepare_v2(IPlist_db,sqlcmd,128, &stmt, NULL);
	sqlite3_step(stmt);
    sqlite3_reset(stmt);
	sqlite3_finalize(stmt);
...
void hostnamelookup_timeout(char* timeout)
{
	FILE* fp;
	int tmp = atof(timeout) * 60 * 60;
    fp = fopen(HOSTNAME_TIMEOUT_SET,"w");
    fprintf(fp,"%d",tmp);
    fclose(fp);
}
