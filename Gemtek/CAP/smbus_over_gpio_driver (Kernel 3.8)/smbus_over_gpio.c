...
static ssize_t bb_smbus_charging_current_read(char *buffer, char **buffer_location, off_t offset, int buffer_length, int *eof, void *data);
...
static int __init gpio2smbus_init( void )
{
  	struct proc_dir_entry *p, *p2, *p3;

	//printk(KERN_DEBUG "gpio2smbus: gpio2smbus_init entry\n" );	

  	p = create_proc_entry(PROC_ENTRY_NAME, 0, 0);
  	if (!p) {
		printk(KERN_DEBUG "[ERROR] Cannot create /proc/bb_smbus_reg\n");
  	}else{
  	...
  	p2 = create_proc_entry(CONNECTED_BATTERY_NAME, 0, 0);
  	if (!p2) {
		printk(KERN_DEBUG "[ERROR] Cannot create /proc/bb_smbus_battery_connected\n");
  	}else{
		p2->read_proc = bb_smbus_battery_connected_read;
		printk(KERN_DEBUG "[Create] Success create /proc/bb_smbus_battery_connected\n");
  	}

  	p3 = create_proc_entry(CHARGING_CURRENT_NAME, 0, 0);
  	if (!p3) {
		printk(KERN_DEBUG "[ERROR] Cannot create /proc/bb_smbus_charging_current\n");
  	}else{
		p3->read_proc = bb_smbus_charging_current_read;
